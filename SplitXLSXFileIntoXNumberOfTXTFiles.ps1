...
$checkThisFolder = "\\ABC-server\Public\Data\Files\"
$archiveFolder = "\\ABC-server\Public\Data\Files\Archive\"
$targetFolder = "\\ABC-server\Public\Data\Files\SplitIntoX"
...
ForEach ($thisFile in $files)
{
  ForEach ($thisX in $xArray)
  {
    ...
    $flattenedCommand = '& "\\ABC-server\Public\Apps\Flattener.exe" + $thisX.substring(0,3) + '.abc' #.substring is 0 and 3 because each item in the array is 4 characters
